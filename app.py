...
if MATPLOTLIB_AVAILABLE:
    ...
    # Fallback: improved text visualization (all strings properly quoted)
    def create_text_viz(query: str) -> str:
        q = query.lower()
        if any(word in q for word in ['exercise', 'yoga', 'pose', 'stretch']):
            return """
**🧘 Visualization (Yoga – Mountain Pose)**  
*Stand tall, feet together, arms at sides.*
"""
        elif any(word in q for word in ['knee', 'back', 'shoulder', 'neck']):
            part = next((w for w in ['knee', 'back', 'shoulder', 'neck'] if w in q), 'body')
            return f"""
**📍 Focus on: {part.capitalize()}**  
🔴 *Pay attention to your {part} area.*
"""
        elif any(word in q for word in ['diet', 'nutrition']):
            return """
**🥗 Healthy Plate Guide** 
🔴 *Pay attention to your {part} area.*
"""
